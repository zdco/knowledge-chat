...
描述的目的是让 AI 分析日志时快速定位问题方向，应从业务视角描述，包含以下维度：

1. **核心职责**：这个服务在业务链路中做什么
2. **数据流向**：数据从哪来（上游）、到哪去（下游），用什么方式传输（TCP、共享内存、UDP 组播、消息队列等）
3. **外部依赖**：连接了什么外部资源（数据库、缓存、第三方服务）
4. **关键配置**：影响运行行为的核心配置文件（如市场映射、路由规则）

**禁止写入描述的内容**：
- 代码级细节：类名、方法名、函数签名（如 `CMultiMarketIf::AddRecvMarket`）
- 版本号（如 v1.0.5）
- 实现细节：内部数据结构、设计模式

这些信息属于代码层面，AI 排查时通过 `scan_service` 从代码中实时获取，不需要写死在描述里。
...
好的描述：
```
接收多个行情源（VSS）的外盘原始行情数据，按市场进行转码后分发给下游服务；支持多市场连接管理和故障自动转移，通过 marketcode.xml 配置市场映射关系
```

不好的描述（包含代码细节）：
```
多市场行情分发服务，通过CMultiMarketIf接口(Init/Start/AddRecvMarket/DelRecvMarket)接收多个VSS的原始行情数据并进行市场级别转码；支持市场连接管理和故障转移，使用marketcode.xml配置市场映射，提供MCvtMonitorObj管理接口
```
