...
- 新增日志分析专用工具：read_log（日志过滤读取）、switch_service（加载服务代码 worktree）、list_services（列出已注册服务）、scan_service（扫描代码依赖线索，替代静态 depends_on）
- 服务注册表（services.yaml）：定义微服务名称、仓库路径、语言，支持 AI 通过对话自动生成
...
- services.yaml 新增 `businesses` 段：按业务线对服务分组，`list_services` 输出按业务线分组显示
...
- log-analyzer 的 session ID 随对话历史一起持久化，切换历史对话或刷新页面后不会丢失
...
- log-analyzer 模式下移除 write_file 工具，search 工具描述改为"在已加载的服务代码中搜索"
- 上传文件按钮从纯图标（📎）改为带文字的按钮（📎 上传文件），更直观易识别
- AI_GUIDE_ANALYZER.md 重写：批量/单个服务注册流程简化为 clone 扫描自动生成，服务描述从业务视角多维度生成，新增刷新描述流程和排除目录规则
...
- git clone/fetch HTTP 地址时禁止弹出交互式认证提示（GIT_TERMINAL_PROMPT=0），避免进程卡住
